...
This file contains full project context for AI agents and Claude Code working on this repository.
...
bun run lint             # Run Biome linting & formatting
...
- `config.ts` — Site configuration (email, social links)
...
- **Email destination:** `contact@jacksdevfolio.com`
...
- **Biome:** Linter & formatter replacing ESLint; runs via `bun run lint`
...
- **Lean architecture:** No unused barrel index files; components imported directly.
- **Single source of truth:** All content in `lib/content.ts`, all config in `lib/config.ts`.
- **Dependencies:** All 19 prod + 11 dev deps actively used; no dead weight.
